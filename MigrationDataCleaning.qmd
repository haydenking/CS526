...
CT = read.delim(paste0(wd,"2024_Gaz_counties_national.txt"), stringsAsFactors = FALSE) |>
  filter(USPS == "CT") |>
...
```{r}
#Add Urban Influence Codes
# https://www.ers.usda.gov/data-products/urban-influence-codes
#maybe population density would be better, will try both
# library(readxl)
uicodes <- read_excel("UrbanInfluenceCodes2013.xls") |>
  rename(fips = FIPS,
         uic = UIC_2013) |>
  select(fips, uic)

irs_data <- left_join(irs_data, uicodes |>
                    rename(receiver_FIPS = fips, uic_receiver = uic))

irs_data <- left_join(irs_data, uicodes |>
                    rename(sender_FIPS = fips, uic_sender = uic))

areas = rbind(read.delim(paste0(wd,"2020_Gaz_counties_national.txt"), stringsAsFactors = FALSE),
read.delim(paste0(wd,"2024_Gaz_counties_national.txt"), stringsAsFactors = FALSE)) |>
  group_by(GEOID) |>
  slice_head(n=1) |>
  ungroup() |>
  mutate(fips = sprintf("%05d", GEOID)) |>
  rename(sqmi = ALAND_SQMI) |>
  select(fips, sqmi)

irs_data  = irs_data |>
  left_join(areas |> rename(sender_FIPS = fips, sender_sqmi = sqmi)) |>
  mutate(sender_density = sender_pop/sender_sqmi) |>
  select(-sender_sqmi)

irs_data  = irs_data |>
  left_join(areas |> rename(receiver_FIPS = fips, receiver_sqmi = sqmi)) |>
  mutate(receiver_density = receiver_pop/receiver_sqmi) |>
  select(-receiver_sqmi)
```
